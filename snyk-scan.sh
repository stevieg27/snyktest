...
snyk_scan(){
    ...
    if [[ "$htmlReport" == "1" ]]; then
        ...
        if [[ "${scanMode}" == "test" ]]; then
            file_name="${rootDir}/snyk-results/$(echo $projectName | tr '/' '_').json"
            #file_name="snyktest.json"
            snyk $scanMode --file="${1}" --project-name="${projectName}" --remote-repo-url="${projectGroup}" $extraArgs --json-file-output="${file_name}"
        elif [[ "${scanMode}" == "monitor" ]]; then
            snyk $scanMode --file="${1}" --project-name="${projectName}" --remote-repo-url="${projectGroup}" $extraArgs
        else 
            echo "Scan mode not supported [Allow only test or monitor]"
        fi
    fi
    return $?
}
